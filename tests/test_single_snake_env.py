...
class TestSingleSnakeEnv(unittest.TestCase):
    def test_basic_movement(self):
        ...
        env.envs = get_test_env(size, 'up')
        actions = torch.Tensor([0, 0, 3, 0, 0, 1]).unsqueeze(1).long()
    ...
    def test_eat_food(self):
        env = SingleSnakeEnvironments(num_envs=1, size=size)
        env.envs = get_test_env(size, 'up')
        actions = torch.Tensor([0, 3, 3, 0, 0, 1]).unsqueeze(1).long()
    ...
    def test_hit_boundary(self):
        env = SingleSnakeEnvironments(num_envs=1, size=size)
        env.envs = get_test_env(size, 'up')
        actions = torch.Tensor([1, ] * 10).unsqueeze(1).long()
    ...
    def test_hit_self(self):
        env = SingleSnakeEnvironments(num_envs=1, size=size)
        env.envs = get_test_env(size, 'up')
        actions = torch.Tensor([0, 3, 3, 2, 1, 0, 0, 0]).unsqueeze(1).long()
    ...
    def test_cannot_move_backwards(self):
        env = SingleSnakeEnvironments(num_envs=1, size=size)
        env.envs = get_test_env(size, 'up')
        actions = torch.Tensor([2, 2, 2, 3]).unsqueeze(1).long()
        expected_head_positions = torch.Tensor([
            [6, 4],
            [7, 4],
            [8, 4],
