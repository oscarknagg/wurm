...
class MultiAgentToSingleAgent(VecEnv):
    """This env takes a MultiAgent env and some agents and makes it appear that it is a single agent
    environment where the other agents are part of the environment (they don't learn).
    """
    def __init__(self, multiagent_env: MultiagentVecEnv, other_agents: List[nn.Module]):
        super(MultiAgentToSingleAgent, self).__init__(multiagent_env.num_envs, multiagent_env.num_agents,
                                                      multiagent_env.height, multiagent_env.width)
        self._env = multiagent_env
        self.other_other_agents = other_agents
    ...
    def render(self, mode: str = 'human', env: Optional[int] = None):
        self._env.render(mode, env)

    def check_consistency(self):
        if hasattr(self._env, 'check_consistency'):
